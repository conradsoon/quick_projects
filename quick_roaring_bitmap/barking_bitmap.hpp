...
auto convertForCardinality(bb_variant const &data) -> bb_variant &&
{
...
auto convert_bset_to_array(bb_bset const &&bset) -> bb_array
{
	bb_array result;
	result.reserve(BB_ARRAY_THRESHOLD);
...
auto convert_array_to_bset(bb_array const &&array) -> bb_bset
{
	bb_bset result;
